...
from sko.PSO import PSO # 导入粒子群优化算法
...
class FilterConfig:
    ...
    def __init__(self):
        ...
        # 电感配置 (单位: nH, 0表示不接电感)
        self.inductors = {
            "L_input": 1.0,  # 输入端口与第一个谐振器之间的电感
            "L_output": 2.0,  # 最后一个Tee与输出端口之间的电感
            "L_fp1": 0.5,  # 并联谐振器fp1与接地之间的电感
            "L_fp2": 0.6,  # 并联谐振器fp2与接地之间的电感
            "L_fp3": 0.4,  # 并联谐振器fp3与接地之间的电感
            "L_fp4": 0.3  # 并联谐振器fp4与接地之间的电感
        }

        # 滤波器性能目标
        self.targets = [
            # S11目标 - 在通带内应小于等于-15 dB
            FilterTarget("回波损耗目标1", "S11", (4.556, 4.796), -15, "max", weight=1.0),
            FilterTarget("回波损耗目标2", "S11", (4.556, 4.796), -10, "max", weight=3.0),

            # S21目标 - 在通带内应大于等于-3 dB
            FilterTarget("插入损耗目标1", "S21", (4.556, 4.796), -3, "min", weight=2.0),
            FilterTarget("插入损耗目标2", "S21", (4.556, 4.796), -1, "min", weight=1.0),

            # S21目标 - 在阻带内应小于等于-30 dB
            FilterTarget("阻带抑制目标1", "S21", (4.3, 4.521), -30, "max", weight=1),
            FilterTarget("阻带抑制目标2", "S21", (4.831, 5.3), -30, "max", weight=1),
            FilterTarget("阻带抑制目标3", "S21", (4.521, 4.536), -20, "max", weight=3),
            FilterTarget("阻带抑制目标4", "S21", (4.816, 4.831), -20, "max", weight=3),
        ]
        ...
        self.resonator_params = {
            # 四个串联谐振器参数
            "fs1": np.array([3554, 1335, 1.68]),
            "fs2": np.array([3554, 1330, 1.35]),
            "fs3": np.array([3554, 1330, 1.63]),
            "fs4": np.array([3554, 1330, 1.48]),

            # 四个并联谐振器参数
            "fp1": np.array([3554, 1619, 1.72]),
            "fp2": np.array([3554, 1563, 1.25]),
            "fp3": np.array([3554, 1563, 1.33]),
            "fp4": np.array([3554, 1574, 1.12])
        }
    ...
    def _define_optimization_params(self):
        ...
        # 变量范围和步长
        resonator_lb = [3300, 1100, 0.1]
        resonator_ub = [3800, 1600, 5.0]
        resonator_step = [5, 5, 0.01]
...
def main():
    ...
    print(f"脚本开始时间: {script_start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # 加载模型和归一化参数
    model, norm_params = load_model_and_params(config.model_dir)
    if model is None or norm_params is None:
        print("无法加载模型或参数，退出程序。")
        return

    # optimizer.set_model_and_params(model, norm_params) # 将模型和参数传递给优化器

    # --- 粒子群优化 (PSO) ---  <- 修改点
    print("\n" + "=" * 60)
    print("开始粒子群优化 (PSO)") # <- 修改点
    print("=" * 60)

    pso_start_time = datetime.datetime.now() # <- 修改点

    # 配置粒子群优化参数 (需要根据 sko.PSO 文档和你的需求调整) <- 修改点
    pso = PSO(func=optimizer.evaluate, # 目标函数
              n_dim=n_dim,         # 变量维度
              pop=40,              # 粒子数量 (示例值, 可调整)
              max_iter=150,        # 最大迭代次数 (示例值, 可调整)
              lb=lb,               # 变量下界
              ub=ub,               # 变量上界
              w=0.8,               # 惯性权重 (示例值)
              c1=0.5,              # 个体学习因子 (示例值)
              c2=0.5               # 全局学习因子 (示例值)
              # 注意: PSO 不直接支持离散步长 'precision'，它处理连续变量。
              # 目标函数内部的参数转换仍然会处理离散化。
             )

    # 运行粒子群优化
    best_x, best_y = pso.run() # <- 修改点

    pso_end_time = datetime.datetime.now() # <- 修改点
    pso_time = pso_end_time - pso_start_time # <- 修改点

    print(f"\n粒子群优化完成，耗时: {pso_time.total_seconds():.2f}秒") # <- 修改点
    print(f"总评估次数: {optimizer.eval_count}")

    # --- 结果处理 ---
    # 从 optimizer 获取记录的最佳结果
    ...
    if best_resonator_params is not None and best_inductor_params is not None:
        ...
        # 修改文件名以反映 PSO
        output_file = os.path.join(output_dir, f"best_params_pso_{timestamp}.txt") # <- 修改点
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(f"优化算法: 粒子群优化 (PSO)\n") # <- 修改点
        ...
        # --- 保存最佳参数为 NPZ 文件 ---
        npz_output_file = os.path.join(output_dir, f"best_params_pso_{timestamp}.npz")
        try:
    ...
    if best_filter_network is not None:
        ...
        print("\n绘制最佳滤波器响应:")
        # 修改图表标题
        plot_filter_s_params(best_filter_network, title=f"优化后的滤波器 (PSO - Score: {best_score:.4f})") # <- 修改点

        # 绘制收敛曲线 (PSO自带)
        plt.figure()
        # 使用 pso.gbest_y_hist 绘制收敛曲线
        plt.plot(pso.gbest_y_hist, 'b-', linewidth=2) # <- 修改点
        plt.title("粒子群优化收敛曲线") # <- 修改点
        plt.xlabel("迭代次数")
        plt.ylabel("最佳加权MSE")
        plt.grid(True)
        plt.show()
    ...
    print(f"脚本总运行时间: {script_duration.total_seconds():.2f} 秒")
