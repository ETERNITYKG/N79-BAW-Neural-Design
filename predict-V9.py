...
# V8 滤波器所有组成部分已构建完成 修正S参数曲线大于0的部分，完成后并入主线
# V9 保存所有谐振器和滤波器的S2P参数
...
class FilterConfig:
    ...
    def __init__(self):
        ...
        # 电感配置 (单位: nH, 0表示不接电感)
        self.inductors = {
            "L_input": 0.456,  # 输入端口与第一个谐振器之间的电感
            "L_output": 0.653,  # 最后一个Tee与输出端口之间的电感
            "L_fp1": 1.188,  # 并联谐振器fp1与接地之间的电感
            "L_fp2": 0.923,  # 并联谐振器fp2与接地之间的电感
            "L_fp3": 2.548,  # 并联谐振器fp3与接地之间的电感
            "L_fp4": 0.808  # 并联谐振器fp4与接地之间的电感
        }

        # # 电感配置 (单位: nH, 0表示不接电感)备份
        # self.inductors = {
        #     "L_input": 3.321,  # 输入端口与第一个谐振器之间的电感
        #     "L_output": 0.904,  # 最后一个Tee与输出端口之间的电感
        #     "L_fp1": 2.83,  # 并联谐振器fp1与接地之间的电感
        #     "L_fp2": 2.897,  # 并联谐振器fp2与接地之间的电感
        #     "L_fp3": 2.839,  # 并联谐振器fp3与接地之间的电感
        #     "L_fp4": 1.481  # 并联谐振器fp4与接地之间的电感
        # }
        ...
        self.resonator_params = {
            # 四个串联谐振器参数
            "fs1": np.array([1624, 3153, 1.45]),
            "fs2": np.array([1574, 3153, 1.55]),
            "fs3": np.array([1624, 3153, 9.25]),
            "fs4": np.array([1624, 3153, 1.25]),

            # 四个并联谐振器参数
            "fp1": np.array([1900, 3153, 2.5]),
            "fp2": np.array([1893, 3153, 0.95]),
            "fp3": np.array([1893, 3153, 0.3]),
            "fp4": np.array([1942, 3153, 1.7])
        }
...
def save_to_s2p(network, filename):
    """
    将网络参数保存为S2P文件，使用幅度/相位格式
    ...
    # 创建文件头
    header = f"! Created {current_time}\n"
    header += "# hz S ma R 50\n"
    header += "! 2 Port Network Data\n"
    header += "! freq\tmagS11\tangS11\tmagS21\tangS21\tmagS12\tangS12\tmagS22\tangS22\n"
    header += "!\n"
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(len(freq)):
            ...
            # 使用科学计数法表示频率，其他数据使用固定小数位数
            f.write(f"{freq[i]:.6e}\t{s11_mag:.9f}\t{s11_ang:.9f}\t{s21_mag:.9f}\t{s21_ang:.9f}\t"
                    f"{s12_mag:.9f}\t{s12_ang:.9f}\t{s22_mag:.9f}\t{s22_ang:.9f}\n")

    print(f"S参数已保存到文件: {filename}")
...
class FilterOptimizer:
    ...
    def _predict_all_resonators(self, resonator_params=None):
        ...
        for params, name in zip(resonator_params, resonator_names):
            try:
                ...
                # 保存谐振器的S参数到S2P文件
                save_to_s2p(network, f"{name}_resonator.s2p")
    ...
    def evaluate_params(self, param_dict=None, inductor_dict=None):
        ...
        # 保存滤波器的S参数到S2P文件
        save_to_s2p(self.filter_network, "filter_complete.s2p")
    ...
    def plot_current_filter(self):
        ...
        plot_filter_s_params(self.filter_network, title="滤波器性能参数")
