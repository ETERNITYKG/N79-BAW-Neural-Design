...
def main():
    ...
    # 配置模拟退火参数
    sa = SA(func=optimizer.evaluate,  # 目标函数
            x0=lb + (ub - lb) * np.random.rand(n_dim),  # 随机初始点
            T_max=100,  # 初始温度
            T_min=1e-3,  # 终止温度
            L=300,  # 每个温度下的迭代次数 (马尔可夫链长度)
            max_stay_counter=150,  # 最大停滞次数
            lb=lb,
            ub=ub,
            step=step  # 指定步长以处理离散变量
            )
    ...
    print(f"\n模拟退火优化完成，耗时: {sa_time.total_seconds():.2f}秒")
    ...
    if best_resonator_params is not None and best_inductor_params is not None:
        ...
        # 修改文件扩展名为 .txt
        output_file = os.path.join(output_dir, f"best_params_sa_{timestamp}.txt")
        try:
            # 使用 'w' 模式打开文件进行写入
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(f"优化算法: 模拟退火 (SA)\n")
        ...
        # --- 保存最佳参数为 NPZ 文件 ---
        npz_output_file = os.path.join(output_dir, f"best_params_sa_{timestamp}.npz")
        try:
    ...
    if best_filter_network is not None:
        ...
        # 绘制最佳滤波器响应
        print("\n绘制最佳滤波器响应:")
        plot_filter_s_params(best_filter_network, title=f"优化后的滤波器 (SA - Score: {best_score:.4f})")

        # 绘制收敛曲线 (SA自带)
        plt.figure()
        plt.plot(sa.generation_best_Y, 'b-', linewidth=2)
        plt.title("模拟退火收敛曲线")
        plt.xlabel("迭代次数")
        plt.ylabel("最佳加权MSE")
        plt.grid(True)
